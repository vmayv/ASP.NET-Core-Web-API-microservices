    ...
    public class NetworkMetricsApiRepository : INetworkMetricsApiRepository
    {
        ...
        public DateTimeOffset GetLastTime(int agentId)
        {
            using (var connection = new SQLiteConnection(SQLParams.ConnectionString))
            {
                try
                {
                    var response = connection.QuerySingle<NetworkMetricApi>("SELECT MAX(Time) FROM networkmetrics WHERE agentId = @agentId", new { agentId = agentId });
        ...
        public IList<NetworkMetricApi> GetMetricFromDatabase(int agentId, DateTimeOffset fromTime, DateTimeOffset toTime)
        {
            using (var connection = new SQLiteConnection(SQLParams.ConnectionString))
            {
                return connection.Query<NetworkMetricApi>("SELECT id, value, time, agentid FROM networkmetrics WHERE agentid=@agentId AND time BETWEEN @fromTime AND @toTime", new { agentId = agentId, fromTime = fromTime.ToUnixTimeSeconds(), toTime = toTime.ToUnixTimeSeconds() }).ToList();
            }
        }
    }
}
