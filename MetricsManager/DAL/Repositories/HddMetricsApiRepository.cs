...
namespace MetricsManager.DAL.Repositories
{
    ...
    public class HddMetricsApiRepository : IHddMetricsApiRepository
    {
        ...
        public IList<HddMetricApi> GetMetricFromDatabase(int agentId, DateTimeOffset fromTime, DateTimeOffset toTime)
        {
            using (var connection = new SQLiteConnection(SQLParams.ConnectionString))
            {
                return connection.Query<HddMetricApi>("SELECT id, value, time, agentid FROM hddmetrics WHERE agentid=@agentId AND time BETWEEN @fromTime AND @toTime", new { agentId = agentId, fromTime = fromTime.ToUnixTimeSeconds(), toTime = toTime.ToUnixTimeSeconds() }).ToList();
            }
        }
    }
}
